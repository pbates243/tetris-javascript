document.addEventListener('DOMContentLoaded', () => {
    ...
    //draw function
    function draw() {
        current.forEach(index => { sqaures[currentPosition + index].classList.add('tetromino')})
    ...
    function undraw()
    {
        current.forEach(index => { sqaures[currentPosition + index].classList.remove('tetromino')})
    }
    ...
    const upNextTetrominos = [
        ...
        [0,1, displayWidth, displayWidth*2+1], //otetris
    ...
    function addScore()
    {
        for(let i=0; i<199; i += width)
        {
            ...
            if (row.every(index => sqaures[index].classList.contains('taken')))
            {
                ...
                const squaresRemoved = sqaures.splice(i, width)
